...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_TIMER:
    {
        if (!IsWindow(g_Minecraft_BE_HWND))
            g_Minecraft_BE_HWND = FindWindow(0, "Minecraft");
        ...
        if (!currentCursorInfo.flags && foregroundHWND == g_Minecraft_BE_HWND)
        {
            ...
            if (g_Minecraft_BE_Rect.top < 0)
                g_Minecraft_BE_Rect.top += 40;
            else
                g_Minecraft_BE_Rect.top += 33;
