...
RECT g_Minecraft_BE_Win_Rect = { 0 };

int g_win_title_offset = GetSystemMetrics(SM_CYSIZE) + GetSystemMetrics(SM_CYSIZEFRAME) * 2 + 1;
int g_win_horizontal_offset = GetSystemMetrics(SM_CXFRAME) * 2;
int g_win_vertical_offset = GetSystemMetrics(SM_CYSIZEFRAME) * 2;
	...
	switch (message) {
		...
		case WM_TIMER: {
			...
			if (!currentCursorInfo.flags && foregroundHWND == g_Minecraft_BE_HWND) {
				g_Disabled = 0;
				GetWindowRect(g_Minecraft_BE_HWND, &g_Minecraft_BE_Win_Rect);

				g_Minecraft_BE_Win_Rect.left += g_win_horizontal_offset;
				g_Minecraft_BE_Win_Rect.right -= g_win_horizontal_offset;
				if (g_Minecraft_BE_Win_Rect.top < 0)
					g_Minecraft_BE_Win_Rect.top += 40;
				else
					g_Minecraft_BE_Win_Rect.top += g_win_title_offset;
				g_Minecraft_BE_Win_Rect.bottom -= g_win_vertical_offset;
				ClipCursor(&g_Minecraft_BE_Win_Rect);
			} else if (!g_Disabled) {
				g_Disabled = 1;
				ClipCursor(0);
			}
